...
export class TaskService extends BaseService {
  ...
  public async getAllTasks(userId: string, projectId?: string): Promise<Task[]> {
    try {
      ...
      let q = query(tasksRef, where('assignedId', '==', userId));
      
      // Si se proporciona un projectId, filtrar también por proyecto
      if (projectId) {
        q = query(tasksRef, where('assignedId', '==', userId), where('projectId', '==', projectId));
      }
